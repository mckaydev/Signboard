...
- 회원 사용자는 간판 검색시 간판사진과 가게 정보, 별점과 한줄평이 검색기록 테이블에 저장됩니다.
...
#### 수행 절차
1. 북마크 추가, 삭제, 기록 삭제 기능을 위하여 사용자의 쿠키에 현재 접속한 기능을 저장합니다. (이전검색기록 보기 or 즐겨찾기 보기)
2. 컨트롤러에서 Mybatis를 통해 DB에서 검색 기록을 요청한 사용자의 기록 수를 받아옵니다. (MySQL COUNT *)
3. 범위를 벗어난 요청은 (초과할 때: 끝페이지로, 미만일 때: 첫페이지로) 리다이렉트 시켜줍니다.
4. 쿼리문으로 넘어온 요청 페이지에 해당하는 검색 기록을 Mybatis를 통해 DB에서 불러옵니다. (MySQL LIMIT)
5. 불러온 기록을 Jackson을 이용하여 JSON 파일로 만들어줍니다.
6. JSON 파일을 뷰로 넘겨주고 뷰에서는 자바스크립트의 '.insertBefore'를 이용하여 검색기록을 화면에 동적 생성합니다.

#### DB 테이블 구조
- 회원 사용자마다 'srchhisto' 스키마에 테이블명이 '회원 ID' 로 이루어진 테이블을 갖습니다.  
...
- 테이블 구조는 다음과 같습니다.
    <table>
        ...
        <tr>
            <td></td><td>isBookmarked</td><td>boolean</td>
        </tr>
        <tr>
            <td></td><td>time</td><td>timestamp</td>
        </tr>
        </tbody>
    </table>
