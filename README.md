...
0. 북마크 추가, 삭제, 기록 삭제 기능을 위하여 사용자의 쿠키에 현재 접속한 기능을 저장합니다. (검색기록 보기 or 즐겨찾기 보기)
1. 컨트롤러에서 Mybatis를 통해 DB에서 검색 기록을 요청한 사용자의 기록 수를 받아옵니다. (MySQL COUNT *)
2. 범위를 벗어난 요청은 (초과할 때: 끝페이지로, 미만일 때: 첫페이지로) 리다이렉트 시켜줍니다.
3. 쿼리문으로 넘어온 요청 페이지에 해당하는 검색 기록을 Mybatis를 통해 DB에서 불러옵니다. (MySQL LIMIT)
4. 불러온 기록을 Jackson을 이용하여 JSON 파일로 만들어줍니다.
2. JSON 파일을 뷰로 넘겨주고 뷰에서는 '.insertBefore'를 이용하여 검색기록을 화면에 동적 생성합니다.
